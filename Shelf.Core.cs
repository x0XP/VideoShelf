...
sealed partial class Shelf : Form {
 ...
 public Shelf() {
  ...
  var streamOnline=new Button();Style(streamOnline,"▶ Stream locally",0,0,145);var downloadOnline=new Button();Style(downloadOnline,"↓ Download locally",0,0,155);var copyLink=new Button();Style(copyLink,"Copy link",0,0,105);
  tips.SetToolTip(streamOnline,"Start a temporary torrent transfer only after this click and play it in VideoShelf's embedded player. The streaming cache is removed when the player closes.");
  tips.SetToolTip(downloadOnline,"Start a persistent torrent download only after this click. You choose the destination folder before any media transfer begins.");
  tips.SetToolTip(copyLink,"Copy the selected torrent/magnet link to the clipboard.");
  onlineActions.Controls.AddRange(new Control[]{resLabel,resolution,streamOnline,downloadOnline,copyLink});
  ...
  configure.Click+=delegate{ConfigureOnline();};resolution.SelectedIndexChanged+=delegate{if(onlineMode)RenderOnline();};streamOnline.Click+=delegate{StreamOnline();};downloadOnline.Click+=delegate{DownloadOnline();};copyLink.Click+=delegate{CopyOnline();};onlineFiles.DoubleClick+=delegate{StreamOnline();};onlineFiles.KeyDown+=delegate(object s,KeyEventArgs e){if(e.KeyCode==Keys.Enter){StreamOnline();e.Handled=true;}};
