...
sealed partial class Shelf {
 void ShowOnline(){
  if(current==null)return;onlineMode=true;ClearSearch();SetSort(true);localDetail.Visible=false;onlineDetail.Visible=true;onlineDetail.BringToFront();subtitle.Text="Online results for "+current.Name;onlineSettings=OnlineSettings.Load();
 ...
 async void StartThumbnailLoading(IEnumerable<OnlineResult> displayed){
  ...
  thumbnailScan.Cancel();thumbnailScan.Dispose();thumbnailScan=new System.Threading.CancellationTokenSource();var ct=thumbnailScan.Token;Person target=current;thumbnailsLoading=true;if(onlineMode)status.Text+=" • Loading thumbnails…";
  try{
 ...
 string ThumbnailTip(OnlineResult r){string query=OnlineThumbnailLookup.SearchText(r.Title);if(r.ThumbnailSource.Length>0)return "Thumbnail search: "+query+"\nImage source: "+r.ThumbnailSource;if(r.ThumbnailError.Length>0)return "Thumbnail search: "+query+"\n"+r.ThumbnailError;return "Thumbnail search: "+query;}
 void RenderOnline(){
  ...
  if(onlineSearching)status.Text="Searching online sources for \""+onlineQueryFor+"\"…";else if(onlineError.Length>0)status.Text="Online search unavailable • "+onlineError;else status.Text=onlineFiles.Items.Count+" seeded online results"+(onlineFiles.Items.Count==0?" • No matching results with at least one seeder.":" • Double-click a result to open it");
  if(thumbnailsLoading)status.Text+=" • Loading thumbnails…";else if(thumbnailsPaused)status.Text+=" • Thumbnail lookup paused by the search engine";else StartThumbnailLoading(rows);
 }
 ...
 void OpenOnline(){if(onlineFiles.SelectedItems.Count==0){status.Text="Select an online result first.";return;}var r=(OnlineResult)onlineFiles.SelectedItems[0].Tag;Launch(r.Link);}
 void CopyOnline(){if(onlineFiles.SelectedItems.Count==0){status.Text="Select an online result first.";return;}try{Clipboard.SetText(((OnlineResult)onlineFiles.SelectedItems[0].Tag).Link);status.Text="Result link copied to the clipboard.";}catch(Exception ex){status.Text="Could not copy link: "+ex.Message;}}
