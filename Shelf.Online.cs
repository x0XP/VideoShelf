...
sealed partial class Shelf {
 ...
 string ThumbnailTip(OnlineResult r){string query=OnlineThumbnailLookup.SearchText(r.Title);string header="Metadata only • media is not downloaded by VideoShelf";if(r.ThumbnailSource.Length>0)return header+"\nThumbnail search: "+query+"\nImage source: "+r.ThumbnailSource;if(r.ThumbnailError.Length>0)return header+"\nThumbnail search: "+query+"\n"+r.ThumbnailError;return header+"\nThumbnail search: "+query;}
 void RenderOnline(){
  ...
  if(onlineSearching)status.Text="Retrieving torrent metadata for \""+onlineQueryFor+"\"… no media is being downloaded";else if(onlineError.Length>0)status.Text="Online search unavailable • "+onlineError;else status.Text=onlineFiles.Items.Count+" seeded online results"+(onlineFiles.Items.Count==0?" • No matching results with at least one seeder.":" • Double-click or choose Stream locally to hand the torrent to your local app");
 ...
 void OpenOnline(){if(onlineFiles.SelectedItems.Count==0){status.Text="Select an online result first.";return;}var r=(OnlineResult)onlineFiles.SelectedItems[0].Tag;status.Text="Handing selected torrent to your local handler…";Launch(r.Link);}
