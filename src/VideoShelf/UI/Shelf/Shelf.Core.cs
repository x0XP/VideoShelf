...
sealed partial class Shelf : Form {
 ...
 readonly TextBox libraryFilter=new TextBox(),onlineQuery=new TextBox();readonly ComboBox homeSort=new ComboBox(),sourceFilter=new ComboBox(),categoryFilter=new ComboBox(),resolution=new ComboBox(),languageFilter=new ComboBox();
 ...
 void BuildCollection(){
  ...
  files.SetBounds(24,202,collectionView.Width-48,collectionView.Height-224);files.Anchor=AnchorStyles.Top|AnchorStyles.Bottom|AnchorStyles.Left|AnchorStyles.Right;files.View=View.Details;files.FullRowSelect=true;files.MultiSelect=false;files.HideSelection=false;files.ShowItemToolTips=true;files.BackColor=Color.FromArgb(8,17,25);files.ForeColor=XdolfTheme.Text;files.BorderStyle=BorderStyle.FixedSingle;files.Columns.Add("VIDEO",420);files.Columns.Add("EPISODE",95);files.Columns.Add("TYPE",80);files.Columns.Add("SIZE",110);files.Columns.Add("MODIFIED",160);files.Columns.Add("SUBFOLDER",220);collectionView.Controls.Add(files);
 }
 void BuildSearch(){
  ...
  languageFilter.SetBounds(947,13,128,40);languageFilter.DropDownStyle=ComboBoxStyle.DropDownList;StyleCombo(languageFilter);languageFilter.Items.AddRange(new object[]{"All languages","English","Japanese","Spanish","French","German","Italian","Portuguese","Korean","Chinese","Russian","Hindi","Arabic","Thai","Dutch","Polish","Turkish","Indonesian","Multi-language","Unspecified"});languageFilter.SelectedIndex=1;filterBar.Controls.Add(languageFilter);
  Style(searchOnline,"Search",1087,13,92);searchOnline.Height=40;XdolfTheme.StylePrimary(searchOnline);filterBar.Controls.Add(searchOnline);Style(configure,"Source",1189,13,84);configure.Height=40;filterBar.Controls.Add(configure);
