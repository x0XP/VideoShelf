...
sealed partial class Shelf {
 ...
 void RenderOnline(){
  ...
  else if(rows.Count==0&&onlineResults.Count>0){searchCount.Text="No results match the current filters";SetStatus("Filtered results","Set source, category and resolution to All, or language to All languages, to widen the results.");}
  else if(rows.Count==0){searchCount.Text="No seeded results found";SetStatus("No seeded results","Try a broader search.");}
  else{searchCount.Text="Found "+rows.Count+" result"+(rows.Count==1?"":"s")+" (metadata only)";SetStatus("Ready","Language: "+lang+" • Results are metadata only. No files are downloaded.");}
  if(!onlineSearching&&!thumbnailsLoading&&!thumbnailsPaused&&rows.Count>0)StartThumbnailLoading(rows);
 }
 void SelectOnline(OnlineResult result,OnlineResultCard card){selectedOnline=result;TransferBridge.Prefetch(result);foreach(Control c in onlineCards.Controls){var rc=c as OnlineResultCard;if(rc!=null){rc.Selected=rc==card;rc.Invalidate();}}RenderInspector();}
 ...
 async void StartThumbnailLoading(IEnumerable<OnlineResult> displayed){
  if(thumbnailsLoading||thumbnailsPaused||onlineSearching||section!=ShellSection.Search||IsDisposed)return;string thumbnailContext=onlineQueryFor;var pending=displayed.Where(r=>r.Seeders>0).GroupBy(r=>OnlineThumbnailLookup.CacheKey(r.Title,thumbnailContext),StringComparer.OrdinalIgnoreCase).Select(g=>g.First()).Where(r=>!thumbnailAttempted.Contains(OnlineThumbnailLookup.CacheKey(r.Title,thumbnailContext))).Take(40).ToArray();if(pending.Length==0)return;
  ...
  try{foreach(var row in pending){ct.ThrowIfCancellationRequested();string key=OnlineThumbnailLookup.CacheKey(row.Title,thumbnailContext);thumbnailAttempted.Add(key);try{using(var result=await OnlineThumbnailLookup.Find(row.Title,thumbnailContext,ct)){if(ct.IsCancellationRequested||IsDisposed||target!=current)return;foreach(var same in onlineResults.Where(x=>OnlineThumbnailLookup.CacheKey(x.Title,thumbnailContext)==key)){same.ThumbnailSource=result.Source;same.ThumbnailError=result.Error;}if(result.TemporarilyBlocked){thumbnailsPaused=true;break;}if(result.Image!=null){foreach(var card in onlineCards.Controls.OfType<OnlineResultCard>().Where(c=>OnlineThumbnailLookup.CacheKey(c.Result.Title,thumbnailContext)==key))card.SetPreview(result.Image);if(selectedOnline!=null&&OnlineThumbnailLookup.CacheKey(selectedOnline.Title,thumbnailContext)==key)RenderInspector();}}}catch(OperationCanceledException){thumbnailAttempted.Remove(key);throw;}catch(Exception ex){row.ThumbnailError=ex.Message;}}}
