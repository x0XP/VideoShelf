...
sealed partial class Shelf {
 ...
 async void OpenPerson(Person p){
  ...
  int token=++generation;current=p;collectionTitle.Text=p.Name;collectionPath.Text=p.Path;ShowSection(collectionView,ShellSection.Collections);navCollections.SetActive(true);SetStatus("Scanning collection…","Finding local videos");videos.Clear();files.Items.Clear();RefreshCollectionVisualState();
  int errors=0;var found=await Task.Run(()=>{var list=new List<Video>();var pending=new Stack<string>();pending.Push(p.Path);while(pending.Count>0){var dir=pending.Pop();try{foreach(var child in Directory.GetDirectories(dir))try{if((File.GetAttributes(child)&FileAttributes.ReparsePoint)==0)pending.Push(child);}catch{errors++;}foreach(var f in Directory.GetFiles(dir)){if(!new[]{".mp4",".mkv",".avi",".mov",".wmv",".webm",".m4v",".mpg",".mpeg",".ts",".mts",".m2ts",".3gp",".flv",".vob"}.Contains(Path.GetExtension(f).ToLowerInvariant()))continue;try{var info=new FileInfo(f);string relative=dir.Length==p.Path.Length?"—":dir.Substring(p.Path.Length).TrimStart(Path.DirectorySeparatorChar);list.Add(new Video{Path=f,Name=info.Name,Size=info.Length,Modified=info.LastWriteTime,Relative=relative,MediaInfo=LibraryMediaIntelligence.Analyze(info.Name,p.Name,relative)});}catch{errors++;}}}catch{errors++;}}return list;});
