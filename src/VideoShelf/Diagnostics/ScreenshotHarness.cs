...
static class ScreenshotHarness {
 ...
 public static void CaptureBuiltInOnlineResults(string displayName,string query,string outputPath,int expectedResults){
  string root=Path.Combine(Path.GetTempPath(),"VideoShelf-built-in-online-"+Guid.NewGuid().ToString("N"));try{Directory.CreateDirectory(root);FolderNaming.CreateCollection(root,displayName);Init();using(var shelf=new Shelf()){PrepareWindow(shelf);shelf.PrepareScreenshotLibrary(root);if(!shelf.PrepareBuiltInOnlineScreenshot(displayName,query,expectedResults))throw new Exception("VideoShelf built-in Find online did not return enough seeded results.");shelf.LayoutSearchForCapture(1120,796);Application.DoEvents();if(!shelf.WaitForAllOnlineThumbnailsForCapture(55000))throw new Exception("Online artwork did not finish loading; refusing to save a screenshot containing placeholder artwork.");Settle(shelf);SaveWindow(shelf,outputPath,1366,860);}Verify(outputPath,5000);}finally{try{if(Directory.Exists(root))Directory.Delete(root,true);}catch{}}
 }
...
sealed partial class Shelf {
 ...
 internal bool WaitForAllOnlineThumbnailsForCapture(int milliseconds){
  DateTime deadline=DateTime.UtcNow.AddMilliseconds(Math.Max(0,milliseconds));
  while(DateTime.UtcNow<deadline&&!IsDisposed){
   Application.DoEvents();
   var cardsToCheck=onlineCards.Controls.OfType<OnlineResultCard>().ToArray();
   if(cardsToCheck.Length>0&&!thumbnailsLoading&&cardsToCheck.All(c=>c.Preview!=null)){RenderInspector();Application.DoEvents();return true;}
   if(thumbnailsPaused&&!thumbnailsLoading)return false;
   if(!thumbnailsLoading&&cardsToCheck.Length>0&&cardsToCheck.Any(c=>c.Preview==null))return false;
   Thread.Sleep(75);
  }
  return false;
 }
