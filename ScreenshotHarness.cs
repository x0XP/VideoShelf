...
static class ScreenshotHarness {
 ...
 public static void CaptureBuiltInOnlineResults(string displayName,string query,string outputPath,int expectedResults){
  string root=Path.Combine(Path.GetTempPath(),"VideoShelf-built-in-online-"+Guid.NewGuid().ToString("N"));try{Directory.CreateDirectory(root);FolderNaming.CreateCollection(root,displayName);Init();using(var shelf=new Shelf()){PrepareWindow(shelf);shelf.PrepareScreenshotLibrary(root);if(!shelf.PrepareBuiltInOnlineScreenshot(displayName,query,expectedResults))throw new Exception("VideoShelf built-in Find online did not return enough seeded results.");Settle(shelf);SaveWindow(shelf,outputPath,1366,860);}Verify(outputPath,5000);}finally{try{if(Directory.Exists(root))Directory.Delete(root,true);}catch{}}
 }
