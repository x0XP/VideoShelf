...
static class PortraitLookup {
 ...
 public static string SearchUrl(string name){return "https://duckduckgo.com/?q="+Uri.EscapeDataString(name+" portrait headshot")+"&iax=images&ia=images&kp=1";}
 ...
 public static async Task<PortraitResult> Find(string name,CancellationToken ct){
  ...
  try{
   ...
   // Public search endpoint, not an official image API. Never solve or bypass challenges.
   await Task.Delay(1200,ct).ConfigureAwait(false);
   string html=Encoding.UTF8.GetString(await Get(SearchUrl(name),2*1024*1024,ct).ConfigureAwait(false));
   ...
   string endpoint="https://duckduckgo.com/i.js?l=uk-en&o=json&q="+Uri.EscapeDataString(name+" portrait headshot")+"&vqd="+Uri.EscapeDataString(token.Groups["token"].Value)+"&f=,,,type:photo,,&p=1";
   ...
   string[] words=name.Split(new[]{' ','_','-'},StringSplitOptions.RemoveEmptyEntries);int tried=0;
   foreach(object item in items){ct.ThrowIfCancellationRequested();var row=item as Dictionary<string,object>;if(row==null)continue;
    string title=Value(row,"title");if(!words.All(w=>title.IndexOf(w,StringComparison.OrdinalIgnoreCase)>=0))continue;
    if(++tried>5)break;string image=Value(row,"thumbnail");if(image.Length==0)image=Value(row,"image");
    try{var photo=Decode(await Get(image,5*1024*1024,ct).ConfigureAwait(false));var result=new PortraitResult{Photo=photo,Source=Value(row,"url")};return result;}
    catch(OperationCanceledException){throw;}catch(LookupBlockedException){throw;}catch{}
   }
   return new PortraitResult{Error="No usable matching portrait found. Right-click to choose an image."};
  }catch(LookupBlockedException ex){blockedUntil=DateTime.UtcNow.AddMinutes(10);return new PortraitResult{Error=ex.Message};}catch(OperationCanceledException){if(ct.IsCancellationRequested)throw;return new PortraitResult{Error="Portrait lookup timed out. Right-click to retry."};}catch(Exception ex){return new PortraitResult{Error=ex.Message};}finally{gate.Release();}
 }
}
}
